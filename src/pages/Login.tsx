...
import { saveUserToDatabase, getUserFromDatabase } from "@/lib/database";
  ...
  const handleManualSubmit = async () => {
    ...
    try {
      if (isRegister) {
        ...
        const existing = await getUserFromDatabase(trimmedEmail);
        if (existing) {
        ...
        const user = await getUserFromDatabase(trimmedEmail);
        ...
        if (!user) {
          ...
          localStorage.setItem("user", JSON.stringify({ name: user.name || trimmedEmail.split("@")[0], email: trimmedEmail }));
        }
