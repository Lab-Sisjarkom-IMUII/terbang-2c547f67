...
async function start() {
  ...
  async function ensureImageUrl(image) {
    ...
    if (typeof image === "string" && image.startsWith("data:")) {
      ...
      const pathKey = `prod-${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
      const upload = await supabase.storage.from("product-images").upload(pathKey, parts.buffer, { contentType: parts.mime, upsert: true });
      if (upload.error) return "";
      const pub = supabase.storage.from("product-images").getPublicUrl(pathKey);
